local config = require "SurfSecurityoupbutler.config"
local null = require "SurfSecurityoupbutler.null"
local Chat = require("SurfSecurityoupbutler.chat")
local ChatMember = require("SurfSecurityoupbutler.chatmember")
local Util = require("SurfSecurityoupbutler.util")
...
local function get_alert_text(self, key)
	...
	local alert_text = {
		new_chat_member = i18n("Log every time a user join the SurfSecurityoup"),
		...
		new_chat_photo = i18n("Log when an admin changes the SurfSecurityoup icon"),
		delete_chat_photo = i18n("Log when an admin deletes the SurfSecurityoup icon"),
		new_chat_title = i18n("Log when an admin change the SurfSecurityoup title"),
		pinned_message = i18n("Log pinned messages"),
		blockban = i18n("Log when a user who has been blocked is banned from the SurfSecurityoup on join"),
...
local function doKeyboard_logchannel(self, chat_id)
	...
	local event_pretty = {
		...
		['flood'] = i18n('Flood'),
		['new_chat_photo'] = i18n('New SurfSecurityoup icon'),
		['delete_chat_photo'] = i18n('SurfSecurityoup icon removed'),
		['new_chat_title'] = i18n('New SurfSecurityoup title'),
...
function _M:onCallbackQuery(blocks)
	...
	if blocks[1] == 'logcb' then
		if not msg.from:isAdmin() then
			api:answerCallbackQuery(msg.cb_id, i18n("You are not admin of this SurfSecurityoup"), true)
			return
		end
	...
	if msg.from.chat.type ~= 'private' then
		...
		elseif blocks[1] == 'unsetlog' then
			...
			if log_channel == null then
				msg:send_reply(i18n("_This SurfSecurityoups is not using a log channel_"), "Markdown")
			else
		...
		elseif blocks[1] == 'logchannel' then
			...
			if log_channel == null then
				...
				if not channel_info and err.error_code == 403 then
					msg:send_reply(
						i18n("_This SurfSecurityoup has a log channel saved, but I'm not a member there, so I can't post/retrieve its info_"),
							"Markdown")
				else
					...
					msg:send_reply(i18n('<b>This SurfSecurityoup has a log channel</b>\nChannel: <code>%s</code>')
